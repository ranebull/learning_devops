...
HOMEWORK_RUN=./learning_devops/homeworks/$BRANCH/run.sh
REPO=https://github.com/ranebull/learning_devops.git
...
if [ -f $HOMEWORK_RUN ]; then
	...
	docker exec hw-test bash -c 'echo -=Get versions=-; ansible --version; ansible-lint --version; packer version; terraform version; tflint --version; docker version; docker-compose --version; pwd; ls -al learning_devops/homeworks;'
